...
class Payment
{
    ...
    public function select_all() {
        ...
        $sql = "SELECT 
                    ...
                    b.name as bname
                from 
                    ".TB_PREFIX."payment ap LEFT JOIN 
                    ".TB_PREFIX."payment_types pt ON (ap.ac_payment_type = pt.pt_id) LEFT JOIN 
                    ".TB_PREFIX."invoices iv ON (ap.ac_inv_id = iv.id AND ap.domain_id = iv.domain_id) LEFT JOIN 
                    ".TB_PREFIX."customers c ON (iv.customer_id = c.id AND iv.domain_id = c.domain_id) LEFT JOIN  
                    ".TB_PREFIX."biller b ON (iv.biller_id = b.id AND iv.domain_id = b.domain_id) LEFT JOIN 
                    ".TB_PREFIX."preferences pref ON (iv.preference_id = pref.pref_id AND iv.domain_id = pref.domain_id)
                WHERE 
